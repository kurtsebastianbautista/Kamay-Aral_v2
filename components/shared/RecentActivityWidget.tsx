  ...
  return (
    <div className="flex h-full flex-col rounded-xl border bg-white dark:bg-card shadow-sm">
      <div className="flex items-center justify-between px-4 pt-4 pb-3">
        ...
        <Link href={viewAllHref} className="text-sm text-indigo-600 dark:text-indigo-300 hover:underline">
          View all →
        </Link>
      </div>
      ...
      {logs.length === 0 ? (
        ...
        <div className="max-h-80 space-y-2 overflow-y-auto px-4 pb-4">
          {logs.map((log) => (
            <div
              key={log.id}
              className="flex items-start justify-between gap-3 rounded-xl border bg-white dark:bg-card p-3 shadow-sm"
            >
              <div className="flex items-start gap-2.5 min-w-0">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-indigo-100 dark:bg-indigo-950 text-xs font-bold text-indigo-600 dark:text-indigo-300">
