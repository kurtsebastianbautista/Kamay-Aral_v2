...
export default function AdminSidebar() {
  ...
  const content = (
    <>
      <Link href="/admin/overview" className="text-lg font-bold text-indigo-600 dark:text-indigo-300 px-4 py-4 block">
        Kamay Aral Admin
      </Link>
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="flex items-center justify-between border-b bg-white dark:bg-card px-4 py-3 md:hidden">
        <span className="text-lg font-bold text-indigo-600 dark:text-indigo-300">Kamay Aral Admin</span>
        <button onClick={() => setOpen((o) => !o)} className="text-muted-foreground">
          {open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
        </button>
      </div>
      {open && (
        <div className="flex flex-col border-b bg-white dark:bg-card pb-2 md:hidden">{content}</div>
      )}
      ...
      <aside className="hidden md:flex md:w-70 md:flex-col md:border-r md:bg-white dark:bg-card md:h-screen md:sticky md:top-0 md:overflow-y-auto">
        {content}
      </aside>
    </>
  )
