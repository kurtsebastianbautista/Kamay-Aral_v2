...
export default async function ProgressPage() {
  ...
  return (
    <div className="px-4 pt-8 pb-4 space-y-6">
      <div>
        <h1 className="text-2xl text-foreground font-black">Progress</h1>
      ...
      <div>
        <div className="space-y-3">
          {MODULES.map((mod) => {
            ...
            return (
              <div key={mod.id} className="flex items-center gap-3 rounded-xl bg-[#ECE7DF] dark:bg-[#2C2116] p-4 border border-[#DAD2C5] dark:border-[#4A3B2A]">
                <span className="text-2xl">{mod.icon}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-foreground font-bold truncate">{mod.title}</p>
      ...
      {attempts && attempts.length > 0 && (
        <div>
          ...
          <div className="space-y-2">
            {attempts.map((a) => (
              <div key={a.submodule_id} className="flex items-center justify-between rounded-xl border bg-white dark:bg-card p-3 shadow-sm">
